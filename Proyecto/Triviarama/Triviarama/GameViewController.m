...
- (void)viewDidLoad {
    ...
    timerDate= [formatter dateFromString:@"00:00"];
    points=0;
    wrongMoves=0;
    [self setBackgrounds];
...
-(void) evaluateCards{
    if([[pairs objectForKey: cardPair1.desc] isEqualToString:cardPair2.desc] || [[pairs objectForKey:cardPair2.desc] isEqualToString:cardPair1.desc]) {
        ...
        winnerCounter++;
        if (winnerCounter==10) {
            [self.resultButton sendActionsForControlEvents: UIControlEventTouchUpInside];
        }
    }
    else {
...
- (void)prepareForSegue:(UIStoryboardSegue *)segue sender:(id)sender
{
    if([[segue identifier] isEqualToString:@"segueToResult"])
    {
        ...
        [[segue destinationViewController] setDataOfScore:dataOfScore];
    } else {
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error {
    ...
    UIAlertView *alerta = [[UIAlertView alloc] initWithTitle:@"Error" message:@"Error to connect" delegate:self cancelButtonTitle:@"OK" otherButtonTitles:nil, nil];
